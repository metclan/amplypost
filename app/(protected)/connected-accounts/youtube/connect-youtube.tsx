...
const youtubeConnectUrl = `${config.backendUrl}accounts/youtube/connect`;
...
export default function YouTubeCallback() {
    ...
    useEffect(() => {
        ...
        async function connectYouTube() {
            if (!code) {
                setState("error");
                setMessage("No authorization code was received from YouTube.");
                return;
            }

            try {
                const response = await fetch(youtubeConnectUrl, {
                    method: "POST",
                    headers: {
                ...
                setState("success");
                setMessage(data?.message || "Your YouTube channel has been connected.");
            } catch (err) {
                console.error("Error during YouTube authorization:", err);
                setState("error");
                setMessage("We could not connect your YouTube channel.");
    ...
    if (error) {
        ...
        return (
            <StatusPanel
                ...
                message={
                    isUserCancelled
                        ? "You cancelled the YouTube connection request. You can try again whenever you are ready."
                        : errorDescription || "YouTube returned an error before the channel could be connected."
                }
            />
        );
    }
