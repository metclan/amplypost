...
const facebookConnectUrl = `${config.backendUrl}accounts/facebook/connect`;
...
export default function FacebookCallback() {
    ...
    useEffect(() => {
        ...
        async function connectFacebook() {
            if (!code) {
                setState("error");
                setMessage("No authorization code was received from Facebook.");
                return;
            }

            try {
                const response = await fetch(facebookConnectUrl, {
                    method: "POST",
                    headers: {
                ...
                console.error("Error during Facebook authorization:", err);
                setState("error");
                setMessage("We could not connect your Facebook account.");
    ...
    if (error) {
        ...
        return (
            <StatusPanel
                ...
                message={
                    isUserCancelled
                        ? "You cancelled the Facebook connection request. You can try again whenever you are ready."
                        : errorDescription || "Facebook returned an error before the account could be connected."
                }
            />
        );
    }
