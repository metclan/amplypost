...
const linkedinConnectUrl = `${config.backendUrl}accounts/linkedin/connect`;
...
export default function ConnectLinkedIn() {
    ...
    useEffect(() => {
        ...
        async function connectLinkedIn() {
            if (!code) {
                setState("error");
                setMessage("No authorization code was received from LinkedIn.");
                return;
            }

            try {
                const response = await fetch(linkedinConnectUrl, {
                    method: "POST",
                    headers: {
                ...
                setState("success");
                setMessage(data?.message || "Your LinkedIn account has been connected.");
            } catch (err) {
                console.error("Error during LinkedIn authorization:", err);
                setState("error");
                setMessage("We could not connect your LinkedIn account.");
    ...
    if (error) {
        ...
        return (
            <StatusPanel
                ...
                message={
                    isUserCancelled
                        ? "You cancelled the LinkedIn connection request. You can try again whenever you are ready."
                        : errorDescription || "LinkedIn returned an error before the account could be connected."
                }
            />
        );
    }
