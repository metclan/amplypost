...
import { apiFetch, backendApiUrl } from "@/util/backend-api";
...
export default function InstagramDownloader() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await apiFetch("tools/instagram-downloader", {
                method: "POST",
                            ...
                            <div key={index} className="mx-auto flex w-full max-w-[280px] flex-col space-y-3">
                                ...
                                {item.thumbnail && (
                                    <div className="relative overflow-hidden rounded-xl border border-border bg-muted aspect-[9/16]">
                                        ...
                                        <img
                                            src={backendApiUrl(`proxy-image?url=${encodeURIComponent(item.thumbnail)}`)}
                                            alt={`Media ${index + 1}`}
